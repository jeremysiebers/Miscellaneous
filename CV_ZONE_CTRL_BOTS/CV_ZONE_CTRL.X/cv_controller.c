...
#define CV_ON_DELAY 10                          //(2 min)-> 10sec (BOTS)        // Delay before enabling the Central Heater
#define LAST_VALVE_OFF_DELAY 600                //(10 min)                      // When the last valve is true an extra off delay is added to allow circulation and after run of the central heater buil in pump
    ...
    for (index = 0; index < 12; index++)                                        // Check all 12 termostats if a valve must be anabled
    {
        switch (cvctrl_IO[index].exec)
        {
            ...
            case    LAST    :   if ((last_valve == OFF) || (TERMOSTATxACTUAL(index) == ON)) // When the last_valve is disabled during a last valve run OR the termostat of the current valve is ON again
                                {                                                           // (this must be checked because the current ternmostat value is not read in the IDLE state)
